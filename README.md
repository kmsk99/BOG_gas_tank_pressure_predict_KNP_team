...
TI_P_MAX	재액화된 LNG 절대 온도를 탱크 내부 최대 압력으로 나눈 값 
(PV = nRT => T/P = V/nR)
TI_VOL_MAX	재액화된 LNG의 부피 상관값
(T/P x n = V/nR x n =V/R)
...
DAYOFYEAR_sin	날짜를 1월 15일 기준으로 삼각함수 변환(연중 최저/최고 기온 날짜)

DAYOFYEAR_cos	
HOUR_sin	시간을 12시 24분을 기준으로 삼각함수 변환(속초의 태양 남중시간)
